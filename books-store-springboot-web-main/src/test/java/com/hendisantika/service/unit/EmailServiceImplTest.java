...
//    }
    ...
    @Test
    void test_send_RuntimeExceptionCaught() {
        MimeMessage mimeMessage = new MimeMessage((Session) null);
        when(mailSender.createMimeMessage()).thenReturn(mimeMessage);
        when(templateEngine.process(anyString(), any(Context.class))).thenReturn("<html></html>");

        // Use RuntimeException instead of MessagingException
        doThrow(new RuntimeException("Simulated")).when(mailSender).send(mimeMessage);

        // Actually it will propagate, so you can wrap it in assertThrows if you want
        Assertions.assertThrows(RuntimeException.class, () -> emailService.send(mail));
    }
